        ...
        <div className="flex justify-center gap-12 flex-wrap">
          {[
            ...
            <div key={stat.label} className="text-center">
              <div className="text-white/50 text-xs tracking-widest uppercase mb-1">{stat.label}</div>
        ...
        {loading ? (
          ...
          <div>
            <div className="flex items-center justify-between px-6 py-3 rounded-2xl">
              {PLANETS.map((_, i) => (
                <div key={i} className="skeleton-shimmer rounded-full" style={{ width: BIG_SIZE, height: BIG_SIZE }} />
              ))}
            </div>
            <div className="flex items-center justify-center gap-1 px-6 py-4 rounded-2xl">
              {PLANETS.map((_, i) => (
                <div key={i} className="skeleton-shimmer rounded-full" style={{ width: SMALL_SIZE, height: SMALL_SIZE }} />
              ))}
            </div>
          </div>
        ) : (         
          <div style={{ border: "1px dashed rgba(151, 64, 255, 0.3)" }}>
            {/* Big planet row */}
            <section>
              <div className="flex items-center justify-between px-6 py-3 rounded-2xl">
                {PLANETS.map((planet) => (
